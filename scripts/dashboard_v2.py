...
def _build_lulc_map(base_path):
    """
    Build a Plotly Scattermap showing both seasonal LULC maps.
    Reads from pre-processed CSV files (created by lulc_map_processor.py).
    Falls back to a placeholder if files are not yet available.
    """
    ...
    if not rabi_loaded and not kharif_loaded:
        # Placeholder: show only the block centre
        fig.add_trace(go.Scattermap(
...
# APP
# ============================================
# Build the heavy Land Cover map exactly once at server startup
        ...
        dcc.Tabs(
            ...
            children=[
                ...
                dcc.Tab(label='🛰 Field Ops', value='fieldops',
                        style={**tab_style(),
                               'borderTop': f"2px solid {COLORS['accent2']}"},
                ...
                dcc.Tab(label='🗺 Land Cover', value='landcover',
                        style={**tab_style(),
                               'borderTop': f"2px solid {COLORS['success']}"},
                        selected_style={**tab_selected_style(),
...
def update_nav(n):
    ...
    ndvi_col  = (COLORS['danger']
                 if ndvi < 0.35 else COLORS['success'])
...
def render_page(tab, n):
        ...
        soil_stats = html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    ...
                    for r_name, r_val in [
                        ('Soil Degradation',
                         risk['degradation_risk'].mean()
                         if not risk.empty else 0),
                        ('Crop Failure',
                         risk['crop_failure_risk'].mean()
                         if not risk.empty else 0),
                        ('Salinity Stress',
                         risk['salinity_risk'].mean()
                         if not risk.empty else 0),
                        ('Overall Risk',
                         risk['overall_risk'].mean()
                         if not risk.empty else 0)
                    ]
                ])
            ], style={**CARD_STYLE, 'flex': '1', 'marginRight': '16px'}),
    ...
    # ==========================================
        ...
        return html.Div([
            ...
            html.Div([
                ...
                dcc.Dropdown(
                    id='soil-prop',
                    options=[{'label': p.replace('_', ' ').title()
                                       + ' (' + v['unit'] + ')',
                              'value': p}
            ...
            html.Div([
                html.Div([
                    dcc.Graph(id='soil-map-v2', style={'height': '520px'})
    ...
    # ==========================================
        ...
        for zone in zones:
            if zone in zone_colors_map:
                fig.add_trace(go.Scatter(
                    ...
                    marker=dict(symbol=zone_markers.get(zone, 'circle'),
                                size=9)
                ))
        ...
        fig.update_layout(
            ...
            yaxis=dict(range=[0, 1.0], title='NDVI',
                       gridcolor=COLORS['border']),
            title=dict(
                text='Real Sentinel-2 NDVI Time Series '
                     '(2025–2026) — Khamanon Block',
        ...
        return html.Div([
            ...
            html.Div('15 months · Real Sentinel-2 · '
                     '4 zones · 5-day revisit',
                     style={'color': COLORS['text_muted'],
                ...
                html.Div([
                    ...
                    html.Span(ndvi_status,
                              style={'fontWeight': '700', 'fontSize': '14px',
                                     'color': COLORS['text']}),
                    html.Span(f"  ·  Block NDVI: {ndvi_mean:.3f}"
                              f"  ·  S2 Date: {s2_date}",
                              style={'color': COLORS['text_muted'],
                                     'fontSize': '12px'})
                ])
    ...
    # ==========================================
        ...
        return html.Div([
            ...
            html.Div('208 cLHS sample points  ·  '
                     'Coloured by pH value  ·  '
                     'Hover for soil profile',
                     style={'color': COLORS['text_muted'],
    ...
    # ==========================================
        ...
        if not shap.empty and 'pH' in shap.index:
            ...
            fig_shap = go.Figure(go.Bar(
                ...
                orientation='h',
                marker=dict(color=shap_pH.values,
        ...
        fig_acc.add_hline(y=0.5, line_dash='dash',
        ...
        soil_num = soil[['pH', 'OC', 'EC', 'K2O',
                          'available_P', 'available_N', 'CEC']]
        ...
        return html.Div([
            ...
            html.Div('SHAP explainability  ·  '
                     'Model validation  ·  Soil correlations',
                     style={'color': COLORS['text_muted'],
                            'fontSize': '12px', 'marginBottom': '20px'}),
            html.Div([
                html.Div([html.Div(shap_chart, style=CARD_STYLE)],
                         style={'flex': '1', 'marginRight': '16px'}),
                html.Div([html.Div([dcc.Graph(figure=fig_acc)],
                                   style=CARD_STYLE)],
                         style={'flex': '1'})
    ...
    # ==========================================
    # TAB 7 — FIELD OPERATIONS MONITOR  (NEW)
    # ==========================================
    elif tab == 'fieldops':

        # ── Load field ops data files ─────────────────────────
        ts_path  = os.path.join(base, '..', 'data',
                                'multiindex_timeseries_clean.csv')
        ev_path  = os.path.join(base, '..', 'data',
                                'field_events.csv')
        fst_path = os.path.join(base, '..', 'data',
                                'field_ops_status.json')
            ...
            return html.Div([
                html.Div('Field Operations data not found.',
                         style={'color': COLORS['danger'],
                                'fontSize': '16px',
                                'marginBottom': '8px'}),
                html.Div('Run field_ops_detector.py first, '
        ...
        fops_st = (json.load(open(fst_path))
                   if os.path.exists(fst_path)
                   else {})

        # ── Determine current phase from latest index values ──
        ...
        if not ev_df.empty:
            ...
            last_label    = EV_LABEL.get(last_ev['event'],
                                         last_ev['event'])
            last_date_str = last_ev['date'].strftime('%b %d, %Y')
            last_col      = EV_COLOR.get(last_ev['event'],
                                         COLORS['accent'])
        else:
        ...
        # ── COMBINED CHART: timeseries + event timeline ───────
        ...
        # Season background shapes in row 1
        season_bands = [
            ('2025-01-01', '2025-05-15', '#1D9E75'),
            ('2025-06-01', '2025-11-30', '#378ADD'),
            ('2025-12-01', '2026-05-26', '#1D9E75'),
        ]
        for x0, x1, sc in season_bands:
            fig_main.add_shape(
                type='rect',
                xref='x', yref='y',
                x0=x0, x1=x1,
                y0=-0.25, y1=0.92,
                fillcolor=sc, opacity=0.07,
        ...
        fig_main.add_trace(go.Scatter(
            x=ts_df['date'], y=ts_df['NDVI'], name='NDVI',
            mode='lines',
        ...
        # NBR
        fig_main.add_trace(go.Scatter(
            x=ts_df['date'], y=ts_df['NBR'], name='NBR',
            mode='lines',
        ...
        # BSI
        fig_main.add_trace(go.Scatter(
            x=ts_df['date'], y=ts_df['BSI'], name='BSI',
            mode='lines',
        ...
        # Event vertical lines in timeseries
        if not ev_df.empty:
            for _, erow in ev_df.iterrows():
                ...
                fig_main.add_shape(
                    type='line',
                    xref='x', yref='y',
                    x0=erow['date'], x1=erow['date'],
                    y0=-0.25, y1=0.92,
                    line=dict(color=ec, width=1.5, dash='dot'),
                    opacity=0.85,
                    row=1, col=1
                )

            # Event dots in timeline subplot (row 2)
            # Only show rows that have detected events
            active_ytypes = ev_df['event'].unique()
            for etype in EV_COLOR:
                if etype not in active_ytypes:
                    continue
                sub = ev_df[ev_df['event'] == etype]
                sizes = [18 if c == 'HIGH' else 12
                         for c in sub['confidence']]
                fig_main.add_trace(go.Scatter(
                    x=sub['date'],
        ...
        if not ev_df.empty:
            for _, erow in ev_df.iterrows():
                ...
                cc    = (COLORS['success']
                         if erow['confidence'] == 'HIGH'
                         else COLORS['warning'])
                table_rows.append(html.Tr([
                    html.Td(
                        erow['date'].strftime('%d %b %Y'),
                        style={'color': COLORS['text_muted'],
                               'padding': '10px 14px',
                               'whiteSpace': 'nowrap',
                               'fontSize': '13px'}
        ...
        event_table = html.Div([
            html.Div(
                'Detected field operations — complete log',
            ...
            html.Table([
                html.Thead(html.Tr([
                    ...
                    for h in ['Date', 'Field Operation',
                              'Confidence', 'Scientific basis']
                ])),
    ...
    # ==========================================
        ...
        if not os.path.exists(lulc_path):
            return html.Div([
                html.Div('Land Cover data not found.', style={
        ...
        ev_t = (pd.read_csv(ev_t_path)
                if os.path.exists(ev_t_path)
                else pd.DataFrame())
        ...
        lc_cards = html.Div([
            lc_card('Block Area',
                    f"{lulc['block_area_ha']:,.0f} ha",
                    'Total Khamanon Block',
                    COLORS['accent']),
            lc_card('Rabi Dominant',
                    f"Wheat {rabi['classes']['Wheat']['pct']}%",
                    f"{rabi['classes']['Wheat']['area_ha']:,.0f} ha · Nov–May",
                    '#fbbf24'),
            lc_card('Spring Maize',
                    f"{rabi['classes']['Spring_Maize']['area_ha']:,.0f} ha",
                    f"{rabi['classes']['Spring_Maize']['pct']}% of Rabi season",
                    '#16a34a'),
            lc_card('Kharif Dominant',
                    f"Rice {kharif['classes']['Rice']['pct']}%",
                    f"{kharif['classes']['Rice']['area_ha']:,.0f} ha · Jun–Nov",
                    '#22c55e'),
            lc_card('Kharif Maize',
                    f"{kharif['classes']['Kharif_Maize']['area_ha']:,.0f} ha",
                    f"{kharif['classes']['Kharif_Maize']['pct']}% of Kharif season",
                    '#fb923c'),
        ], style={
        ...
        fig_compare.add_trace(go.Bar(
            x=rabi_classes,
            y=rabi_areas,
            marker_color=rabi_colors,
            text=[f"{a:,.0f} ha<br>{rabi['classes'][c]['pct']}%"
        ...
        fig_compare.add_trace(go.Bar(
            x=kharif_classes,
            y=kharif_areas,
            marker_color=kharif_colors,
            text=[f"{a:,.0f} ha<br>{kharif['classes'][c]['pct']}%"
        ...
        fig_rot.update_layout(
            ...
            plot_bgcolor=COLORS['card'],
            height=280,
            margin=dict(l=220, r=120, t=30, b=40),
            font=dict(color=COLORS['text']),
            xaxis=dict(title='Estimated area (ha)',
                       gridcolor='rgba(255,255,255,0.05)'),
        ...
        if not ev_t.empty:
            for _, erow in ev_t.iterrows():
                cc  = CLASS_COLOR_MAP.get(str(erow.get('crop_class', '')),
                                          COLORS['text_muted'])
                cfr = (COLORS['success']
                ...
                ev_rows.append(html.Tr([
                    ...
                    html.Td(
                        html.Span(
                            ...
                            style={'backgroundColor': cfr + '22',
                                   'color': cfr, 'padding': '3px 10px',
                    ...
                    html.Td(
                        html.Span(str(erow.get('crop_class', '')),
                    ...
                    html.Td(
                        f"{erow.get('area_ha', 0):,.0f} ha",
                        style={'color': COLORS['text'], 'padding': '10px 14px',
                               'fontSize': '13px', 'fontWeight': '600'}
                    ),
                    html.Td(
                        html.Span(str(erow.get('confidence', '')),
            ...
            html.Table([
                ...
                html.Tbody(ev_rows if ev_rows else [
                    html.Tr([html.Td(
                        ...
                        colSpan=6,
                        style={'color': COLORS['text_muted'],
                               'padding': '20px 14px', 'fontSize': '13px'}
                    )])
                ])
            ], style={'width': '100%', 'borderCollapse': 'collapse'})
        ], style=CARD_STYLE)
        ...
        return html.Div([
            ...
            html.Div(
                '2 seasonal LULC maps  ·  19,616 ha  ·  '
                'Sentinel-2 + SAR fusion  ·  '
            ...
            html.Div([
                html.Div([
                    html.Span('\U0001f5fa Land Cover Map', style={
                        'color'     : COLORS['text'],
                        'fontSize'  : '14px',
                        'fontWeight': '600'
                    }),
                    html.Span(
                        '  \u00b7  Khamanon Block  '
                        '\u00b7  Toggle seasons in legend  '
                        '\u00b7  Hover for class detail',
                        style={'color': COLORS['text_muted'],
                               'fontSize': '12px'}
                    )
                ...
                dcc.Graph(
                    id='lulc-map',
                    figure=_lulc_map_cache,  # Uses the pre-built map instantly
...
def update_map(prop, n):
    ...
    fig  = go.Figure(data=go.Densitymap(
        ...
        showscale=True,
        colorbar=dict(
            title=dict(text=units.get(prop, ''),
                       font=dict(color=COLORS['text'])),
            tickfont=dict(color=COLORS['text']),
            thickness=12
        ),
    ...
    context = {
        'pH'         : 'Mean 8.39 — strongly alkaline. '
                       'Values >8.5 risk micronutrient lock-up.',
        'OC'         : 'Mean 0.43% — critically low. '
                       'Healthy soil needs >0.75%.',
        'EC'         : 'Mean 0.10 dS/m — no salinity stress. '
                       'Safe for all crops.',
        'available_N': 'Mean 225 kg/ha — moderate. '
                       'High spatial variability detected.',
        'available_P': 'Mean 30.5 kg/ha — moderate. '
                       'PAU optimal: 25-35 kg/ha.',
        'K2O'        : 'Mean 71 kg/ha — some deficiency '
                       'zones detected (<55 kg/acre).',
        'CEC'        : 'Mean 14 meq/100g — typical for '
                       'loamy alluvial Punjab soils.',
        'bulk_density': 'Mean 234 g/L — some compaction '
                        'zones above 260 g/L.',
        'CaCO3'      : 'Mean 0.76% — locks up P and '
                       'micronutrients in high zones.'
    }
